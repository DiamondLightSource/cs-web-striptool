    ...
    curves.forEach((curve: Curve, idx: number) => {
      curveOpts.push(
        <Tr key={curve.name}>
          ...
          <Td>
            <Checkbox
              isDisabled={!isModify[idx]}
              onChange={e => handleChange(e, "plotStatus", idx)}
              isChecked={curve.plotStatus}
            />
          </Td>
          <Td>
            <Checkbox
              isDisabled={!isModify[idx]}
              onChange={e => handleChange(e, "scale", idx)}
              isChecked={curve.scale}
            />
          </Td>
          <Td>
            {isModify[idx] ? (
              <Input
                onChange={e => handleChange(e, "precision", idx)}
                height="30px"
                defaultValue={curve.precision.toString()}
              />
            ) : (
              curve.precision
            )}
          ...
          <Td>
            {isModify[idx] ? (
              <Input
                onChange={e => handleChange(e, "min", idx)}
                height="30px"
                defaultValue={curve.min.toString()}
              />
            ) : (
              curve.min
            )}
          ...
          <Td>
            {isModify[idx] ? (
              <Input
                onChange={e => handleChange(e, "max", idx)}
                height="30px"
                defaultValue={curve.max.toString()}
              />
            ) : (
              curve.max
            )}
          ...
          <Td>
            <Button onClick={() => handleClick(idx)}>
              {isModify[idx] ? "Save" : "Modify"}
            </Button>
          </Td>
          <Td>
            <Button onClick={() => dispatch(deleteCurve(curve.name))}>
              Remove
            </Button>
          </Td>
        </Tr>
