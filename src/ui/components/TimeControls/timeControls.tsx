    ...
    return (
        <>
          <SimpleGrid columns={2} spacing={2}>
            ...
            {isModify ? <Input onChange={(e) => handleChange(e, "timespan")} height='30px' defaultValue={time.timespan.toString()} /> : <Center height='30px'> {time.timespan}</Center>}
            <Center height='30px'> Ring Buffer Size</Center>
            {isModify ? <Input onChange={(e) => handleChange(e, "numSamples")} height='30px' defaultValue={time.numSamples.toString()} /> : <Center height='30px'> {time.numSamples}</Center>}
            <Center height='30px'> Data Sample Interval</Center>
            {isModify ? <Input onChange={(e) => handleChange(e, "sampleInterval")} height='30px' defaultValue={time.sampleInterval.toString()} /> : <Center height='30px'> {time.sampleInterval}</Center>}
            <Center height='30px'> Graph Redraw Interval</Center>
            {isModify ? <Input onChange={(e) => handleChange(e, "refreshInterval")} height='30px' defaultValue={time.refreshInterval.toString()} /> : <Center height='30px'> {time.refreshInterval}</Center>}
          </SimpleGrid>
          <Button onClick={handleClick} colorScheme="blue" variant="outline">{isModify ? "Save" : "Modify"}</Button>
        </>
    );
}
