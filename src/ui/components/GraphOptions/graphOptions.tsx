import { Center, Radio, RadioGroup, Select, SimpleGrid, Slider, SliderFilledTrack, SliderThumb, SliderTrack, Stack } from "@chakra-ui/react";
...
export const GraphOptions = (): JSX.Element => {
    ...
    const handleChange = (event: React.ChangeEvent<HTMLSelectElement> | React.MouseEvent<HTMLSelectElement>, name: string) => {
    ...
    return (
        <SimpleGrid columns={2} spacing={2}>
            ...
            <Select onChange={(e) => handleChange(e, "option.gridXOn")} defaultValue={option.gridXOn.toString()}>
                <option value="0">None</option>
                <option value="1">Some</option>
                <option value="2">All</option>
            </Select>
            <Center height='30px'> y-grid lines:</Center>
            <Select onChange={(e) => handleChange(e, "option.gridYOn")} defaultValue={option.gridYOn.toString()}>
                <option value="0">None</option>
                <option value="1">Some</option>
                <option value="2">All</option>
            </Select>
            <Center height='30px'> y-axis label color:</Center>
            <RadioGroup onChange={() => dispatch(modifyConfig("option.axisYColorStat", !option.axisYColorStat))} value={option.axisYColorStat.toString()}>
                <Stack direction='row'>
            ...
            <Slider onChange={(val) => dispatch(modifyConfig("option.graphLineWidth", val))} aria-label='slider-ex-1' defaultValue={2} min={1} max={10}>
                <SliderTrack>
                    <SliderFilledTrack />
                </SliderTrack>
                <SliderThumb />
