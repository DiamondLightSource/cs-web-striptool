...
export const GraphPanel = (): JSX.Element => {
  const timespan: number = useSelector((state: StripToolConfig) => state.time.timespan);
  const [labels, unit, tickValue, multiplier] = computeTimeTickLabels(timespan);
  return (
      <Box sx={{width: "100%"}}>
        <Legend></Legend>
        <Graph xLabelStrings={labels} xAxisUnit={unit} xTickValue={tickValue} xTickMultiplier={multiplier}></Graph>
      </Box>
    );
  }